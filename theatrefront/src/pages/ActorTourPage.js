...
export default function ActorTourPage({
    ...
    const init = ()  => {
        actorTourService.getAll(currentPage - 1, recordPerPage)
            .then(response => {
                console.log('ActorTour data', response.data);
                setActorTour(response.data.content);
                    ...
                    <tbody>
                    {
                        actorTour.map(obj => (
                            <tr key={`${obj.id?.dateID}.${obj.id?.actorID}`}>
                                ...
                                <td style={{ fontSize: "14px" }}>{obj.date.dateStart}</td>
                                <td style={{ fontSize: "14px" }}>{obj.date.dateEnd}</td>

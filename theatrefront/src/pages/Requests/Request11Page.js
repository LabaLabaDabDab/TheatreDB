...
function Request11Page({ history }) {
    ...
    const init = () => {
        ...
        ticketsService.getTicketsDetails(filterData)
            .then(response => {
    ...
    return (
        <div>
            <h2>Подробности о проданных билетах на представления</h2>
            ...
            <Table style={{ width: '100%', marginTop: 20, marginRight: 40, marginLeft: 0 }} striped bordered hover variant="dark">
                ...
                {performanceDetails.map((performanceDetail, index) => (
                    <tr key={index}>
                        <td>{performanceDetail.performanceTitle}</td>
                        <td>{new Date(performanceDetail.dateOfPerformance).toLocaleDateString()}</td>
                        <td>{performanceDetail.soldTicketsCount}</td>
                    </tr>
                ))}
                </tbody>
            </Table>
...
export default withRouter(Request11Page);

...
export default function GenrePage({
    ...
    const init = (currentPage)  => {
        genreService.getAll(currentPage - 1, recordPerPage)
            .then(response => {
                console.log('Author data', response.data);
                setGenre(response.data.content);
    ...
    return (
        <div>
            ...
            <div className={"table-container"}>
                <Link to="/genres/add" style={{ marginLeft: 10, marginTop: 10, color: 'white' }} className="btn btn-dark mb-2">Добавить</Link>
                    ...
                    <tbody>
                    {
                        genres.map(obj => (
                            <tr key={obj.id}>
                                ...
                                <td>
                                    <Link style={{ backgroundColor: "#D10000", borderColor: "#D10000" }} to={`/genres/edit/${obj.id}`} className='btn btn-danger'>Изменить</Link>
